...
Original file is located at
    https://colab.research.google.com/drive/1zOGGlwdm_Oe7g8S9ec_Jn1Gb6DqfIg9l
...
#calculate first fixation duration

# Calculate fixation durations
fixation_durations = first_fix_ms_df['fixation_end_time'] - first_fix_ms_df['fixation_start_time']

# You can add this duration column to your DataFrame if needed
first_fix_ms_df['fixation_duration'] = fixation_durations

# Display the updated DataFrame
display(first_fix_ms_df)
...
fix_cols = ['session', 'trial', 'first_fix_ms','fixation_end_ms', 'fixation_duration', 'frame_time_ms']
...
final_df = final_df[['session', 'trial',
                     'first_fix_ms','fixation_end_ms', 'fixation_duration',
...
#1. elemenate invalid sessions 69, 70, 107, 108
...
# fixation end ends after reaction time
...
#3. Trials continuing after the elementate fixation end reaction

# fixation end ends after reaction time
negatives2 = final_df[final_df['reaction_time_real_ms'] - final_df['fixation_end_ms'] < 0]

print(f"Toplam {len(negatives2)} Trial fixation start finished AFTER reaction and NaN was made")
...
final = ['session', 'trial', 'fixation_duration', 'valence','side']


ffd_final = final_df[final].copy()

ffd_final

#Apply log 10 format for first_fix_ms
ffd_final["log10_fixation_duration"] = np.log10(ffd_final["fixation_duration"] + 1)

ffd_final

# Drop any row that contains a NaN value
ffd_final = ffd_final.dropna().reset_index(drop=True)

print(ffd_final.shape)

# --- Histograms ---
plt.figure(figsize=(12,5))

plt.subplot(1, 2, 1)
sns.histplot(ffd_final['fixation_duration'], bins=50, kde=True, color='blue')
plt.title("First Fixation Duration Distribution")
plt.xlabel("FFD (ms)")
plt.ylabel("Frequency")

plt.subplot(1, 2, 2)
sns.histplot(ffd_final['log10_fixation_duration'], bins=50, kde=True, color='green')
plt.title("Log10 First Fixation Duration Distribution")
plt.xlabel("Log10(FFD)")
plt.ylabel("Frequency")

plt.tight_layout()
plt.show()

mask_2000 = ffd_final[ffd_final['fixation_duration'] > 2000]

print(mask_2000.shape)

mask_2000

for _, row in mask_2000.iterrows():
...
ffd_df_final = ffd_df[["participant","trials","fixation_duration","log10_fixation_duration", "side", "valence"]].copy()
...
# Apply effect coding
ffd_df_final["valence"] = ffd_df_final["valence"].map({
    0 : -1, #negative
    1 : 1 #positive
})

ffd_df_final["side"] = ffd_df_final["side"].map({
    "left": -1,
    "right": 1
})

# Drop any row that contains a NaN value
ffd_df_final = ffd_df_final.dropna().reset_index(drop=True)

plt.figure(figsize=(12,5))

plt.subplot(1, 2, 1)
sns.histplot(ffd_df_final['fixation_duration'], bins=50, kde=True, color='blue')
plt.title("First Fixation Duration Distribution")
plt.xlabel("FFD (ms)")
plt.ylabel("Frequency")

plt.subplot(1, 2, 2)
sns.histplot(ffd_df_final['log10_fixation_duration'], bins=50, kde=True, color='green')
plt.title("Log10 First Fixation Duration Distribution")
plt.xlabel("Log10(FFD)")
plt.ylabel("Frequency")
...
sns.barplot(data=ffd_df_final, x="valence", y="fixation_duration", ci="sd")
plt.title("FFD by Emotional Valence")
plt.xlabel("Emotional Valence (-1: Negative, 1: Positive)")
plt.ylabel("First Fixation Duration (ms)")
plt.show()

sns.barplot(data=ffd_df_final, x="side", y="fixation_duration", ci="sd")
plt.title("FFD by Side ")
plt.xlabel("Side (-1: left, 1: right)")
plt.ylabel("First Fixation Duration (ms)")
plt.show()

ffd_df_final['Valence_Label'] = ffd_df_final['valence'].map({-1: 'Negative', 1: 'Positive'})
ffd_df_final['Side_Label'] = ffd_df_final['side'].map({-1: 'Left', 1: 'Right'})
...
plt.title("FFD by Emotional Valence and First Fixation Side", fontsize=14, weight='bold', pad=15)
plt.xlabel("Emotional Valence", fontsize=12)
plt.ylabel("First Fixation Duration (ms)", fontsize=12)
plt.legend(title="First Fixation Side")
plt.tight_layout()
plt.show()

groups = {
    "Right-Positive": (ffd_df_final["side"] == 1) & (ffd_df_final["valence"] == 1),
    "Right-Negative": (ffd_df_final["side"] == 1) & (ffd_df_final["valence"] == -1),
    "Left-Positive": (ffd_df_final["side"] == -1) & (ffd_df_final["valence"] == 1),
    "Left-Negative": (ffd_df_final["side"] == -1) & (ffd_df_final["valence"] == -1)
}

# Grups mean and standart
summary_df = pd.DataFrame(columns=["Group", "Mean", "Std"])


for name, condition in groups.items():
    subset = ffd_df_final.loc[condition, "fixation_duration"]
...
log10_model = mixedlm("log10_fixation_duration ~ valence + side",
                      data=ffd_df_final,
                      groups=ffd_df_final["participant"],
                      re_formula="~side")
